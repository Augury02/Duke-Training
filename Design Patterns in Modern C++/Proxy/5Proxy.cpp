...
/*
encapsulates the creation and management of Monster objects, providing lazy initialization 
and potentially allowing for additional functionalities in the future

Whereas I used to only have one monster object, which always needs to be created at the start;
I am now able to create multiple monster templates which only creates one when needed
at random
*/
private:
    shared_ptr<Monster> realMonster;

    shared_ptr<Monster> createRandomMonster() {     //Randomizer for which monster will be created
        random_device rd;
        mt19937 eng(rd());
...
bool battle() {
    ...
    while (playerHealth > 0 && monsterHealth > 0) {
        ...
        if (monsterHealth <= 0) {
            cout << "You defeated the " << monster.getName() << "!" << endl;        //edited to suit proxy implementation
            return true;
        }
        ...
        if (monsterAction >= 1 && monsterAction <= static_cast<int>(monsterAttacks.size())) {
            ...
            //edited to suit proxy implementation
